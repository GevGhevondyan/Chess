...
const ChessBoard = ({ onChangeTurn }) => {
    ...
    const checkBishop = (e, rows, checkKing) => {
        let canGo = [];
        let canBeEaten = [];
        const color = checkKing ? e.dataset.color : e.target.dataset.color;
        // STE CONDITIONAL ES DRE CHECK STUGELU HAMAR KARAS CONDITIONY JNJES
        const number = checkKing
            ? e.parentElement.id.slice(1) * 1
            : e.target.parentElement.id.slice(1) * 1;
        const letter = checkKing
            ? e.parentElement.id.slice(0, 1)
            : e.target.parentElement.id.slice(0, 1);
        // MINCHEV STEX
        let ctr = [];
        let ctl = [];
        let cbl = [];
    ...
    // const checkCheck = (e, rows) => {
    //     let blackKing = document.getElementById(bkPos).firstChild;
    //     console.log(e, blackKing);
    //     checkBishop(blackKing, rows, true);
    //     console.log(canEat, checkLegal);
    // };
    ...
    return <div className={"chessBoard " + playingBy}>{renderBoard()}</div>;
};
